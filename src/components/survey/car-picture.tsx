...
export function CarPicture({ color, className = "", eager = false }: { color: CarColor; className?: string; eager?: boolean }) {
  ...
  return (
    <picture className="contents">
      ...
      <img src={`/cars/${slug}-1920.webp`} alt={`${color} / ${COLOR_META[color].english} ET 主驾 45 度视图 / driver-side 45-degree view`} className={className} loading={eager ? "eager" : "lazy"} fetchPriority={eager ? "high" : "auto"} draggable={false} />
    </picture>
  );
}
