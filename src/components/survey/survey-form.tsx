...
const DRAFT_KEY = "ex7-color-survey-draft-v3";
  ...
  return (
    <section ref={surveyRef} id="survey" data-hydrated={hydrated} className="relative z-10 mx-auto max-w-4xl px-5 pb-32 pt-24 md:px-8 md:pb-44 md:pt-36">
      <div className="mb-16 md:mb-24">
        ...
        <p className="mt-6 max-w-xl leading-7 text-white/52">问卷共 {TOTAL_QUESTIONS} 题，预计用时 2 分钟。第 1、2 题为选填，不收集姓名、电话等个人身份信息。</p>
        <p className="mt-2 max-w-xl leading-7 text-white/38">This survey has {TOTAL_QUESTIONS} questions and takes about 2 minutes. Questions 1 and 2 are optional. We do not collect names, phone numbers, or personal identity information.</p>
      </div>
      ...
      <form onSubmit={submit} noValidate className="space-y-5 md:space-y-7">
        <QuestionCard number={1} title={<BilingualText {...QUESTION_TITLES[0]} />} hint="选填 / Optional" error={errors.responsibleCountry}>
        ...
        <QuestionCard number={2} title={<BilingualText {...QUESTION_TITLES[1]} />} hint="选填 / Optional" error={errors.productLine}>
        ...
        <QuestionCard number={3} title={<BilingualText {...QUESTION_TITLES[2]} />} error={errors.ageGroup}>
        ...
        <QuestionCard number={4} title={<BilingualText {...QUESTION_TITLES[3]} />} hint="可多选 / Multiple selection" error={errors.purchasePurposes}>
        ...
        <QuestionCard number={5} title={<BilingualText {...QUESTION_TITLES[4]} />} error={errors.firstImpression}><ColorOptions name="firstImpression" value={draft.firstImpression as CarColor | undefined} onChange={(value) => update("firstImpression", value)} /></QuestionCard>
        <QuestionCard number={6} title={<BilingualText {...QUESTION_TITLES[5]} />} hint="拖动卡片，或使用右侧按钮调整顺序 / Drag the cards or use the buttons on the right to adjust the order." error={errors.ranking}>
        ...
        <QuestionCard number={7} title={<BilingualText {...QUESTION_TITLES[6]} />} error={errors.premiumColor}><ColorOptions name="premiumColor" value={draft.premiumColor as CarColor | undefined} onChange={(value) => update("premiumColor", value)} /></QuestionCard>
