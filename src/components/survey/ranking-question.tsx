...
function SortableColor({ color, index, total, onMove }: { color: CarColor; index: number; total: number; onMove: (from: number, to: number) => void }) {
  ...
  return (
    <div ref={setNodeRef} style={style} className={`flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[.045] p-3 transition-shadow md:p-4 ${isDragging ? "z-20 shadow-2xl shadow-black/50 ring-1 ring-blue-300/50" : ""}`}>
      ...
      <button type="button" className="cursor-grab touch-none rounded-lg p-1.5 text-white/35 hover:bg-white/8 hover:text-white/75 focus-visible:outline-2 focus-visible:outline-blue-300" aria-label={`拖动${meta.display}调整排名`} {...attributes} {...listeners}><GripVertical className="h-5 w-5" /></button>
      <span className="h-3.5 w-3.5 rounded-full ring-4 ring-white/5" style={{ background: meta.hex }} />
      <span className="grid leading-tight">
        <span className="font-medium">{meta.display}</span>
        <span className="text-xs text-white/42">{meta.english}</span>
      </span>
      <div className="ml-auto flex gap-1">
        <button type="button" disabled={index === 0} onClick={() => onMove(index, index - 1)} aria-label={`${meta.display}上移一名`} className="rounded-lg p-2 text-white/55 hover:bg-white/10 disabled:opacity-20"><ChevronUp className="h-4 w-4" /></button>
        <button type="button" disabled={index === total - 1} onClick={() => onMove(index, index + 1)} aria-label={`${meta.display}下移一名`} className="rounded-lg p-2 text-white/55 hover:bg-white/10 disabled:opacity-20"><ChevronDown className="h-4 w-4" /></button>
      </div>
    </div>
  );
}
