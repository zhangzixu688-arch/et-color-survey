    ...
    <section className="glass-strong rounded-[1.6rem] p-5 md:p-7">
      ...
      <div ref={containerRef} className="h-64 min-w-0 w-full">
        {size.width > 0 && size.height > 0 && <BarChart width={size.width} height={size.height} data={data} margin={{ top: 10, right: 2, left: -25, bottom: 0 }}>
            ...
            <XAxis dataKey="name" tick={{ fill: "rgba(255,255,255,.54)", fontSize: 11 }} axisLine={false} tickLine={false} />
            <YAxis tick={{ fill: "rgba(255,255,255,.34)", fontSize: 10 }} axisLine={false} tickLine={false} allowDecimals={false} />
            <Tooltip cursor={{ fill: "rgba(255,255,255,.04)" }} contentStyle={{ background: "rgba(8,12,18,.94)", border: "1px solid rgba(255,255,255,.14)", borderRadius: 14 }} formatter={(value) => [`${value} 人`, "选择人数"]} />
      ...
      <div className="mt-3 grid grid-cols-3 gap-2 text-center">{data.map((item) => <div key={item.name} className="rounded-xl bg-white/[.035] px-2 py-2"><p className="text-[10px] text-white/38">{item.name}</p><p className="mt-1 text-sm font-medium">{item.percentage}%</p></div>)}</div>
    </section>
  );
}
