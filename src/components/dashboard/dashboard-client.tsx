...
import { COLOR_META, getColorDisplayName } from "@/lib/survey";
...
const metricTitles: { key: keyof DashboardStats["colorMetrics"]; title: string; subtitle: string }[] = [
  ...
  { key: "premiumColor", title: "高级感认知", subtitle: "第 7 题 · 品牌形象色" },
  { key: "resaleColor", title: "保值感认知", subtitle: "第 8 题 · 二手保值判断" },
];
    ...
    <main className="relative min-h-screen overflow-hidden bg-[#05070a] px-4 pb-20 pt-6 md:px-8 lg:px-12">
      ...
      <div className="relative z-10 mx-auto max-w-[1600px]">
        ...
        <section className="glass-strong overflow-hidden rounded-[1.8rem]">
          ...
          <div className="scrollbar-thin overflow-x-auto"><table className="w-full min-w-[1700px] text-left text-sm"><thead className="text-xs text-white/35"><tr>{["提交时间","负责国家","产品线","年龄段","购车用途","第一眼","完整排序","高级感","保值","实际购买(旧)","主视觉(旧)"].map((title) => <th key={title} className="px-5 py-4 font-normal">{title}</th>)}</tr></thead><tbody>{pageData.items.map((item) => <tr key={item.id} className="border-t border-white/7 text-white/62 hover:bg-white/[.025]"><td className="whitespace-nowrap px-5 py-4">{new Date(item.createdAt).toLocaleString("zh-CN", { hour12: false })}</td><td className="px-5 py-4">{item.responsibleCountry || "-"}</td><td className="px-5 py-4">{item.productLine || "-"}</td><td className="px-5 py-4">{item.ageGroup}</td><td className="px-5 py-4">{item.purchasePurposes.join("、")}</td><td className="px-5 py-4">{getColorDisplayName(item.firstImpression)}</td><td className="px-5 py-4">{item.ranking.map(getColorDisplayName).join(" › ")}</td><td className="px-5 py-4">{getColorDisplayName(item.premiumColor)}</td><td className="px-5 py-4">{getColorDisplayName(item.resaleColor)}</td><td className="px-5 py-4">{item.purchaseChoice ? getColorDisplayName(item.purchaseChoice) : "-"}</td><td className="px-5 py-4">{item.campaignColor ? getColorDisplayName(item.campaignColor) : "-"}</td></tr>)}{pageData.items.length === 0 && <tr><td colSpan={11} className="px-5 py-16 text-center text-white/35">尚无问卷数据</td></tr>}</tbody></table></div>
